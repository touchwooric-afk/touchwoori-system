...
### 2026-03-21 — Phase 1 이후 추가 구현
- **role 확장**: sub_master(교육목사), auditor(교육위원장) 신규 추가
  - sub_master: 회원관리·역할부여 전담, 재정 개입 불가, master 계정 수정 불가
  - auditor: 전체 부서 장부·영수증·결산 열람 전용, 부서 RLS 우회
- **감사 내역 아카이브**: settlements에 audit_file_url/audit_note 컬럼 추가, 파일 첨부·메모 기능
- **회계장부 메뉴 개편**: "장부" → "회계장부", 엑셀 내보내기 장부 그룹으로 이동
- **장부 정렬 개선**: 날짜→항목명→금액→카테고리 다중 필드 정렬
- **특수 장부 조회 버그 수정**: 관리 페이지 "조회" 버튼이 ledgerId 파라미터 없이 이동하던 문제
- **영수증 업로드 전체 저장 개선**: 유효하지 않은 항목은 error 상태로 보류, 수정 후 재저장 가능
- **DB 마이그레이션**: 002_audit_fields.sql, 003_new_roles.sql
