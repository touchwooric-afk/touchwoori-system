...
import { FileSpreadsheet, Upload, Download, CheckCircle, AlertCircle, XCircle } from 'lucide-react';
...
export default function ExcelPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setOverrides({});
      setExcluded(new Set());
      const unmatchedCount = json.data.preview.filter((r: PreviewRow) => r.isValid && !r.categoryId).length;
      if (unmatchedCount > 0) {
        toast.info(`${json.data.totalRows}행 파싱 완료 — 카테고리 미매칭 ${unmatchedCount}건은 직접 선택해주세요`);
      } else {
        toast.info(`${json.data.totalRows}행 파싱 완료 (유효: ${json.data.validRows}행)`);
      }
    } catch (err) {
  ...
  const importableRows = preview
    ? preview.preview.filter((r) => r.isValid && !excluded.has(r.rowIndex) && getEffectiveCategoryId(r))
    : [];
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <span className="text-sm text-gray-600">
                    ...
                    <span className="text-emerald-600 font-medium">가져오기 {importableRows.length}건</span>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {preview.preview.map((row) => {
                        ...
                        const isUnmatched = row.isValid && !effectiveCatId && !isExcluded;
